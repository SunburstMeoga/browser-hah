        ...
        <div v-if="loadStatus === 'finished'" class="w-11/12 mr-auto ml-auto text-sm sm:hidden">
            <div class="py-3 border-b border-lightborder dark:border-border100" v-for="(item, index) in dataList"
                ...
                <div class="flex justify-start item-center mb-1">
                    <div class="pr-2 text-lighttable dark:text-white200">{{ $t('Rank.address') }}: </div>
                    <div class="text-clickable">{{ addressFilter(item.address) }}</div>
                </div>
                <div class="flex justify-start item-center mb-1">
                    <div class="pr-2 text-lighttable dark:text-white200">{{ $t('Rank.balance') }}: </div>
                    <div class="text-clickable"> {{ item.balance }}</div>
                </div>
                <div class="flex justify-start item-center">
                    <div class="pr-2 text-lighttable dark:text-white200">{{ $t('Rank.yield') }}: </div>
                    <div class="text-clickable">{{ item.yield }}</div>
                </div>
            </div>
        </div>
